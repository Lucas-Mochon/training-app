...
class _WorkoutCreatePageState extends State<WorkoutCreatePage> {
  ...
  Future<void> _handleSubmit() async {
    ...
    if (_selectedGoal == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Veuillez sélectionner un objectif')),
      );
      return;
    }
    ...
    try {
      ...
      await context.read<WorkoutStore>().create(
        userId,
        duration,
        _selectedGoal!,
      );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Entraînement créé avec succès !'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Créer un entraînement'), elevation: 0),
      body: SafeArea(
            ...
            children: [
              ...
              Container(
                ...
                child: DropdownButton<Workoutgoal>(
                  ...
                  hint: const Text('Sélectionner un objectif'),
